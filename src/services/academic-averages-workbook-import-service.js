...
const ANNUAL_AVERAGE_HEADER_ALIASES = {
  studentId: ["رقم الطالب"],
  year: ["السنة الدراسية"],
  // المدرسة غيّرت اسم العمود من "السنوي" لـ"النهائي" بنسخة لاحقة من الملف
  // (رُصد فعليًا: شيت المعدلات السنوية كان يعطي صفرًا صفوفًا بلا هذا البديل،
  // فيغيب المعدل التراكمي لكل الطلبة بصمت بعد الاستيراد).
  finalCumulativeAverage: ["المعدل التراكمي السنوي", "المعدل التراكمي النهائي"],
};
